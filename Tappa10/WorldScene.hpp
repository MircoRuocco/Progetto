...
public:
    ...
    void spawnPortale(sf::Vector2f position)
    {
        m_portali.emplace_back(position.x, position.y);
    }
    ...
    void update(float deltaTime)
    {
        ...
        for (auto& portale : m_portali)
        {
            portale.aggiorna(deltaTime, velocitaLivello);
        }

        for (const auto& portale : m_portali)
        {
            if (!m_player.isNavicella() &&
                m_player.getHitbox().findIntersection(portale.getBounds()))
            {
                m_player.attivaNavicella();
            }
        }
